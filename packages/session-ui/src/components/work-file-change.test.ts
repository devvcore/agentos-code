...
describe("workFileChanges", () => {
  test("uses the basename for edits", () => {
    expect(workFileChanges("edit", { filePath: "/Users/me/Reports/Q3 Summary.xlsx" }, {})).toEqual([
      { file: "Q3 Summary.xlsx", kind: "update" },
    ])
  })

  test("distinguishes created and updated writes", () => {
    const input = { filePath: "/tmp/out/report.docx" }
    expect(workFileChanges("write", input, { exists: false })).toEqual([{ file: "report.docx", kind: "create" }])
    expect(workFileChanges("write", input, { exists: true })).toEqual([{ file: "report.docx", kind: "update" }])
    expect(workFileChanges("write", input, {})).toEqual([{ file: "report.docx", kind: "write" }])
  })
  ...
  test("reads patch metadata files, preferring moved paths", () => {
    expect(
      ...
      { file: "notes.md", kind: "create" },
      { file: "old.csv", kind: "delete" },
      { file: "final.txt", kind: "update" },
    ])
  })
  ...
  test("falls back to patch text headers while pending", () => {
    ...
    expect(workFileChanges("patch", { patchText }, {})).toEqual([
      { file: "plan.md", kind: "create" },
      { file: "budget.csv", kind: "update" },
    ])
  })
})
